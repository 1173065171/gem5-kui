#!/usr/bin/env python3
"""
KuiSau normal-convolution stride+shift test with RTL-style C-read gating.

This config keeps the SAU.py-derived normal_conv_stride_shift output check, but
asks KuiSau to gate C/bias memory reads like RTL mem_addr.sv. The vector uses
work_mode=0 and zero C data, so the functional D output stays unchanged while
the request trace should report no C reads.
"""
...
system.golden = SauGoldenGen(
    ...
    poll_interval=20,
    max_busy_polls=1000,
    test_case="normal_conv_stride_shift",
)
