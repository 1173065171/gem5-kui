#!/usr/bin/env python3
"""
KuiSau deterministic depthwise-convolution stride-mode golden-reference test.

This config runs conv_kernel=2 with register_mode=2 and stride=1.
SauGoldenGen writes one nonzero A lane in input row 1 and checks that the DW
B-column mask leaves only one packed D byte in row 8.
"""
...
system.golden = SauGoldenGen(
    ...
    poll_interval=20,
    max_busy_polls=1000,
    test_case="depthwise_conv_stride",
)
...
print("=" * 80)
print("KuiSau Golden Depthwise-Conv Stride Test")
print("=" * 80)
