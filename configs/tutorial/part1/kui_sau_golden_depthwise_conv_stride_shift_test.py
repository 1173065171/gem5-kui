#!/usr/bin/env python3
"""
KuiSau deterministic depthwise-convolution stride+shift golden-reference test.

This config runs conv_kernel=2 with register_mode=2, stride=1, and
shift_mode=1. SauGoldenGen writes one nonzero A lane in input row 1 and checks
that the DW B-column mask still leaves one packed D byte in the int16 layout.
"""
...
system.golden = SauGoldenGen(
    ...
    poll_interval=20,
    max_busy_polls=1000,
    test_case="depthwise_conv_stride_shift",
)
...
print("=" * 80)
print("KuiSau Golden Depthwise-Conv Stride+Shift Test")
print("=" * 80)
