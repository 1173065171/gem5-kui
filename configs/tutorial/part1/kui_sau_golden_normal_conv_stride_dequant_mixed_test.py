#!/usr/bin/env python3
"""
KuiSau deterministic normal-convolution stride+dequant golden-reference test.

This config runs conv_kernel=2 with register_mode=0, stride=1, work_mode=2,
and cutbit=2. SauGoldenGen writes one nonzero A lane in input row 1, alternating
signed B data, and all-one C bias to check stride row selection plus dequant.
"""
...
system.golden = SauGoldenGen(
    ...
    poll_interval=20,
    max_busy_polls=1000,
    test_case="normal_conv_stride_dequant_mixed",
)
...
print("=" * 80)
print("KuiSau Golden Normal-Conv Stride+Dequant Test")
print("=" * 80)
